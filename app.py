...
# Caricamento variabili (locale e cloud)
load_dotenv()
...
ARCH_DESC = {
    ...
    "The Golden Retriever": "🐶 Pura energia e felicità. Eccessivamente entusiasta, 'zerbino' totale, usa mille emoticon"
}
...
with tab_sim:
    ...
    if not st.session_state.ui_messages:
        ...
        with col2:
            ...
            st.info(ARCH_DESC[archetipo_scelto]) # Descrizione dinamica
        ...
        if st.button(t["start"], use_container_width=True, type="primary"):
            ...
            if easter_egg_triggered:
                ...
                if dinamica == t["desired"]:
                    full_prompt += "\n[DINAMICA]: L'utente è il Desiderato. Tu (IA) devi sedurlo. Inizia tu."
                else:
        ...
        # Chat Attiva
        titolo_c = "⚖️ Easter Egg: Gabri Found!" if st.session_state.is_easter_egg else f"⚖️ Frame-Gym: {st.session_state.archetipo_scelto}"
